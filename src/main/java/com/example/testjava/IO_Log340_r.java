...
/**
IO_Log340_r class was generated for DDL Rec/Def: log340 on: 4/30/24 at: 11:16:12.
This is an INPUT/OUTPUT class which can be used to read and write data.*/

  public class IO_Log340_r implements com.tandem.ext.guardian.GuardianInput, 
...
**/
    public void unmarshal(byte [] _buffer, int _count) 
      ...
      if(_offset >= _count) return;
      _fieldSize = Math.min(15,_count - _offset);
      encodeSetting = System.getProperty("log_risk_rate.encoding",defaultEncoding);
        try {
           log_risk_rate = DataConversion.CobolStrToJavaStr(_buffer,_offset,_fieldSize,encodeSetting);
        }catch (DataConversionException dce) {
           _numConversionErrors++;
           if(_allowErrors) {
...
**/
    ...
    public byte[] marshal(byte [] _buffer)
      ...
      _offset = 65;
        ...
        encodeSetting = System.getProperty("log_risk_rate.encoding",defaultEncoding);
        DataConversion.JavaStrToCobolStr(_buffer,log_risk_rate,_offset,15, encodeSetting);
        }
      return _buffer;
    }
  }
